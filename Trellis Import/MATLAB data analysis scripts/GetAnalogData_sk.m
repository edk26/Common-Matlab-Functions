function [time, AnalogWaveforms, AnalogElectrodeIDs, nsFileInfo] = GetAnalogData_sk(filename, sf, channels)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
...
%
%Edited by Efstathios Kondylis 12/14
%Now channels input dictates which AnalogElectrodeID to use, rather than
%requiring match with AnalogElectrodeID
%line 110 replaces line 109
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     thisID = find(AnalogElectrodeIDs==channels(i));
    thisID=AnalogElectrodeIDs(channels(i));
    if ~isempty(thisID)
        fprintf('Reading electrode %d | %d...\n', channels(i), thisID);
        [~, AnalogSampleCounts(j), rawraw] = ...
            ns_GetAnalogData(hFile, AnalogEntityIDsSF(channels(i)), AnalogSampleReadStart, AnalogSampleReadCount);
        %         A(:,1)=butterysmooth(rawraw,30000,900,1000);
        AnalogWaveforms(:,j)=rawraw;
        j=j+1;
    else
